...
/*
 * The SoundManager class manages sound playback. The SoundManager is a ThreadPool, with each thread playing back one sound at a time. This allows the SoundManager to easily
 * limit the numer of simultaneous sounds being played. 
 * <p>Possible ideas to extend this class:<ul>
 * <li>add a setMasterVolume) method, which uses Controls to set the volume for each line. 
 * <li>don't play a sound if more than, say, 500ms have passed since the request to play
 * </ul>
 */
public class SoundManager extends ThreadPool {
	...
	/*
	 * Signals that a PooledThread has started. Cfreate the Thread's line and buffer.
	 */
	...
	/*
	 * Signals that a PoolThread has stopped. Drains and closes the Thread's line.
	 */
	...
	/*
	 * The SoundPlayer class is a task for the PooledThreads to run. It receives the thread's Line and byte buffer from the ThreadLocal variables and plays a sound
	 * from an InputStream.
	 * <p>This class only works when called from PooledThread.
	 */
	protected class SoundPlayer implements Runnable{
		...
		public void run() {
			//get line and bugger from ThreadLocals
			SourceDataLine line = (SourceDataLine)localLine.get();
