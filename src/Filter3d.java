...
/*
 * The Filter3d class is a SoundFilter that creates 3D sound effect. The sound is filtered so that it is quieter the farther away the sound source is from the listener.
...
//	}
	...
	/*
	 * Creates a new Filtered3d object with the specified source and listener Sprites. The Sprite's position can be changed while this filter is running.
	 * <p>The maxDistance parameter is the maximum distance that the sound can be heard.
	 */
	public Filter3d(final Sprite source, final Sprite listener, final int maxDistance) {
		this.source = source;
		this.listener = listener;
		this.maxDistance = maxDistance;
		this.lastVolume = 0.0f;
