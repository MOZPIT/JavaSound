...
public class EchoFilter extends SoundFilter {
	 ...
	 * <p>The number of decay samples specifies how long before the echo is initially heard. For a 1 second echo with mono,
	 * 44100Hz sound, use 44100 delay samples.
	 * <p>The decay value is how much the echo has decayed from the source. A decay value of .5 means the echo heard is half as loud as the source. 
	 */
	...
	/*
	 * Filters the sound samples to add an echo. The samples played are added to the sound in the delay buffer multiplied by the decay rate. The result is then stored in the 
	 * delay buffer multiplied by the decay rate. The result is then stored in the delay buffer, so multiple echoes are heard.
	 */
